...
func TestApplyAlgoTag_StampsBodyFromEnv(t *testing.T) {
	...
	if po.AlgoID != 162933 {
		t.Fatalf("PlaceOrder AlgoID = %d, want 162933", po.AlgoID)
	}
	b, _ := json.Marshal(po)
	if !strings.Contains(string(b), `"algoId":162933`) {
		t.Fatalf("body must serialize \"algoId\":162933, got %s", b)
	}
}
...
func TestApplyAlgoTag_RespectsExplicitAndEmpty(t *testing.T) {
	...
	if explicit.AlgoID != 999 {
		t.Fatalf("explicit AlgoID overridden: %d", explicit.AlgoID)
	}
}
